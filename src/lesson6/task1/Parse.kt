 ...
 */
fun bestLongJump(jumps: String): Int = if (jumps.contains(Regex("""[^\- %0123456789]"""))) -1
else jumps.split(" ").filter { it.contains(Regex("""\d""")) }.map { it.toInt() }.max() ?: -1
...
fun bestHighJump(jumps: String): Int {
    val regExp = Regex("[0123456789+]")
    var res = -1
    if (jumps.contains(regExp)) {
        val a = jumps.split(" ")
        for (x in a.indices step 2) {
            val b = a[x + 1].last()
            val c = a[x].toInt()
            if (b == '+' && c > res) res = c
        }
...
fun plusMinus(expression: String): Int {
    ...
    if (expression.matches(regExp)) {
        if (a.size > 1) {
            for (x in 1 until a.size step 2) {
                val b = a[x]
                val c = a[x + 1].toInt()
                if (b == operators[0]) result += c
                else result -= c
            }
        }
