...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((name, grade) in grades) {
        a[grade] = a.getOrDefault(grade, mutableListOf()) + name
    }
    return a
}
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((name, phone) in a) {
        if (name in mapB && mapB[name] != phone) {
            a[name] += ", " + mapB[name]
        }
    }
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((stock, price) in stockPrices) {
        if (stock !in a) {
            ...
            a[stock] = a[stock]!! + price
            count[stock] = count[stock]!! + 1
        }
    }
    for ((stock, _) in a) {
        a[stock] = a[stock]!! / count[stock]!!
    }
    return a
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    for (i in list) {
        val a = list.indexOf(i)
        val b = list.indexOf(number - i)
        if ((number - i) in list && a != b) {
            return if (number - i <= i) Pair(b, a)
            else Pair(a, b)
        }
    }
    return Pair(-1, -1)
