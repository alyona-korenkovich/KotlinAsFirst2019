...
fun alignFileByWidth(inputName: String, outputName: String) {
    ...
    for (line in File(inputName).readLines()) {
        ...
        if (words.size <= 1) {
            outputStream.write(words.first())
            outputStream.newLine()
        } else {
            val lineWithSpaces = words.joinToString(" ")
            val n = (maxLineLength - lineWithSpaces.length) / (words.size - 1)
            // минимальное равное количество пробелов между словами, которые необходимо расставить
            var m = (maxLineLength - lineWithSpaces.length) % (words.size - 1)
            // оставшееся количество пробелов после расставления минимума
            if (lineWithSpaces.length == maxLineLength) {
                outputStream.write(newLine)
                outputStream.newLine()
            } else {
                val resLine = mutableListOf<String>()
                for (word in words.subList(0, words.size - 1)) {
                    resLine.add(word)
                    resLine.add(" ".repeat(n + 1))
                    if (m > 0) resLine.add(" ")
                    m--
                }
                resLine.add(words.last())
                outputStream.write(resLine.joinToString(""))
                outputStream.newLine()
            }
        }
    }
